...
WebService.prototype.getResourceExtension = function getResourceExtension(model) {
  return this._models[model].settings.restExtension || '.json';
};
...
WebService.prototype.create = function create(model, data, callback) {
    ...
    $.post(this.getResourceUrl(model) + this.getResourceExtension(model), req, function (res) {
        if (res.code === 200) {
...
WebService.prototype.save = function save(model, data, callback) {
    ...
    req._method = 'PUT';
    req[model] = this.preProcess(data);
    $.post(this.getResourceUrl(model) + '/' + data.id + this.getResourceExtension(model), req, function (res) {
        if (res.code === 200) {
...
WebService.prototype.exists = function exists(model, id, callback) {
    $.getJSON(this.getResourceUrl(model) + '/' + id + this.getResourceExtension(model), function (res) {
        if (res.code === 200) {
            callback(null, true);
...
WebService.prototype.destroy = function destroy(model, id, callback) {
    ...
    var req = this.getBlankReq();
    req._method = 'DELETE';
    $.post(this.getResourceUrl(model) + '/' + id + this.getResourceExtension(model), req, function (res) {
        if (res.code === 200) {
...
WebService.prototype.all = function all(model, filter, callback) {
    var _this = this;
    $.getJSON(this.getResourceUrl(model) + this.getResourceExtension(model) + '?query=' + encodeURIComponent(JSON.stringify(filter)), function (res) {
        if (res.code === 200) {
